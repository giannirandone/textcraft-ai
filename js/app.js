...
class TextCraftApp {
    ...
    updateProcessButton() {
        ...
        this.processBtn.disabled = !(hasText && hasMode);
    }

    async processText() {
        const inputText = this.inputTextarea.value.trim();
        
        if (!inputText || !this.currentMode) {
            return;
        }
        ...
        try {
            ...
            this.displayError('Fehler beim Verarbeiten des Textes. Bitte versuchen Sie es erneut.');
        } finally {
    ...
    simulateAIResponse(text, prompt) {
        ...
        const responses = {
            summarize: `Zusammenfassung: ${text.substring(0, Math.min(100, text.length))}...`,
            correct: text.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
            formal: `Sehr geehrte Damen und Herren,\n\n${text}\n\nMit freundlichen Grüßen`,
            casual: `Hey! 👋\n\n${text}\n\nViele Grüße! 😊`,
            professional: `Betreff: ${text.substring(0, 50)}...\n\n${text}`,
            creative: `✨ ${text.split('').map((char, i) => i % 2 === 0 ? char.toUpperCase() : char).join('')} ✨`,
    ...
    async copyToClipboard() {
        ...
        try {
            ...
            this.copyBtn.innerHTML = '✅ Kopiert!';
